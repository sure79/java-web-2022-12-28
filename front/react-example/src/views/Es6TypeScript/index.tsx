...
export default function Es6Typescript() {
    ...
    //? 문자열을 표현할 때는 '',"",``
    let str: string = '10';
    const description2 = `str값은 ${str}입니다.`;
    ...
    //? |, &
    //? | : 한 변수에 두개 이상의 데이터타입을 지정해 줄 때 사용
    ...
    //? & : 한 변수에 두개의 데이터타입을 속성을 동시에 지정할 때 사용
    interface I1 {
        a:string;
    ...
    //# 5. if
    //? if문의 조건 
    //? ES6 와 TS에서는 false, '', 0 , null, underfined를 모두 false로 받고 나머지는 모두 true
    const emptyArray: null = null;
    if (!emptyArray) console.log ('a');

    //# 6. for
    //? foreach 반복문
    //? Java -> for(요소데이터 타입 변수명 : 반복해서 접근 할 배열){}
    //? TS   -> for (const 변수명 of 반복해서 전급 할 배열) {}
    ...
    // const object5: IExample = new Example('a',1,true)
