...
export default function PopularCard({title} : Props) {
  ...
  return (
    <Card variant ='outlined' sx={{ p: '24px 12px 26px 24px'}}>
        ...
        <Box sx={{ mt : '24px'}}>
         {popularList.length === 0 ? (<Box sx = {{ height : '344px', display: 'flex', justifyContent: 'center', alignItems : 'center'}}><Typography sx={{ fontSize : '24px', fontWeight : 500, color : 'rgba(0,0,0,0.4)' }}>검색결과 없습니다. </Typography></Box>) 
